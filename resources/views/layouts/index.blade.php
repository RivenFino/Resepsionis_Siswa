    ...
    <body class="">
        ...
        <nav class="w-full flex h-24 max-sm:h-20 justify-center items-center fixed top-0 z-10 transition-all ">
            <div class="flex justify-between items-center w-[1400px] h-full max-sm:w-full max-sm:px-4 ">
                ...
                <div class="flex w-full h-full items-center justify-center max-sm:hidden">
                    <a id="openDataBtn" class="hover:cursor px-6 text-2xl flex items-center font-semibold hover:text-red-600 h-full">Data</a>
                    <a id="openFormBtn" class="px-6 text-2xl flex items-center font-semibold hover:text-red-600 h-full">Isi Formulir</a>
                    <a class="px-6 text-2xl flex items-center font-semibold hover:text-red-600 h-full" href="#panduan">Panduan</a>
                ...
                <div id="navModal" class="fixed h-full inset-0 bg-white hidden"> <!-- Nav untuk mobile -->
                    <div class="bg-white flex flex-col justify-center w-full mx-auto rounded-lg">
                        ...
                        <div class="flex flex-col h-auto items-center gap-8 bg-white">
                            <a href="#home" class="text-2xl flex items-center w-full justify-start px-4 border-b py-6  font-semibold hover:text-red-600">Home</a>
                            <a id="openDataBtn" class="hover:cursor text-2xl flex items-center w-full justify-start px-4 border-b py-6  font-semibold hover:text-red-600">Data</a>
                            <a href="Form" class="text-2xl flex items-center w-full justify-start px-4 border-b py-6 font-semibold hover:text-red-600">Isi Formulir</a>
                            <a id="tombolPanduan" href="#panduan" onclick="scrollToElement('panduan')" id="tombolPanduan" class="text-2xl flex items-center w-full justify-start px-4 border-b py-6  font-semibold hover:text-red-600">Panduan</a>
                ...
                <div id="dataForm" class="fixed flex justify-center items-center w-full h-full inset-0 bg-black bg-opacity-50 hidden transition-all"> 
                    <div class="bg-white flex flex-col justify-center w-[500px] pb-12 h-auto mx-auto rounded-xl">
                        ...
                        <div class="w-full h-6 px-8">
                            <h2 class="text-xl flex w-full justify-center text-gray-700 font-semibold">Masukkan NIS Terlebih dahulu</h2>
                        </div>
                        <form action="/Data" enctype="multipart/form-data" method="POST" class="flex flex-col py-8 pt-12 items-center gap-8 bg-white w-full max-sm:px-3 px-8 h-full transition-all">
                            @csrf
                            <div class="flex items-center flex-col justify-center w-3/4 px-8 gap-6">
                        ...
                        <form action="/process" enctype="multipart/form-data" method="POST" class="flex flex-col py-8 pt-12 items-center gap-8 bg-white w-full max-sm:px-3 px-8 h-full transition-all">
                            ...
                            <div class="flex items-center flex-col justify-center w-3/4 px-8 gap-6">
                                <div class="flex max-w-[400px] w-full flex-col">
                                    ...
                                    <input type="number" name="nis" id="nisform" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50 focus:bg-blue-50 focus:outline-none outline-none transition-all" value="">
                                </div>
                                <div class="flex justify-end mt-5 px-8">
                                    <div class="inline-flex items-end">
                                        <button type="submit" class="transition-all w-64 h-12 bg-blue-600 hover:bg-white hover:border hover:border-red-600 hover:text-red-600 text-gray-100 font-bold py-2 px-4 rounded">Simpan</button>
        ...
        <script>
            $(document).ready(function () {
                var nisInput = $('#nisform');
                var namaInput = $('#nama');

                nisInput.on('change', function () {
                    var nis = $(this).val();
                    ...
                    if (nis !== '') {
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data) {
                                    // Mengisi nilai kolom nama dengan nama siswa
                                    namaInput.val(data.nama);
                                } else {
                                    // Jika siswa tidak ditemukan, membersihkan nilai kolom nama
                                    namaInput.val('');
